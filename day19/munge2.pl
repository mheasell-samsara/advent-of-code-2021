...
my @scanner_locations = map {
    my $chain = $transform_tree->{$_};
    my $location = transform_point({x=>0,y=>0,z=>0}, @$chain);
} (0..$#regions);

print Dumper(\@scanner_locations);

my $max_distance = 0;
for my $i (0..$#scanner_locations) {
    for my $j (($i+1)..$#scanner_locations) {
        my $diff = vec_subtract($scanner_locations[$i], $scanner_locations[$j]);
        my $dist = manhattan_distance($diff);
