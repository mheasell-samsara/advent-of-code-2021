...
for my $number (@numbers) {
    ...
    for my $i (0..$#boards) {
        my $board = $boards[$i];
        next if defined $board->{win_number};
    ...
    if (scalar @boards == 1 && defined $boards[0]->{win_number}) {
        my $board = $boards[0];
        my $sum = sum_of_unmarked($board);
        my $score = $number * $sum;
        print "number: $number, sum of unmarked: $sum, score: $score\n";
        exit;
    }

    @boards = grep { not defined($_->{win_number}) } @boards;
